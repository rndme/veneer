//veneer.full.src.js
//95,731b of code compiled Friday, October 17, 2014 11:19:18 PM
	...
	events: {
		...
		var href=""; 
			...
			if(!veneer.$("link[href*='bootstrap']")[0]){
				veneer.$("head")[0].appendChild(veneer.elm("link",{href:"https://netdna.bootstrapcdn.com/bootstrap/3.1.1/css/bootstrap.min.css", rel: "stylesheet" }));
			}
		
			if(elm.icons){
				if(!veneer.$("link[href*='font-awesome']")[0]){
			veneer.$("head")[0].appendChild(veneer.elm("link",{href:"https://netdna.bootstrapcdn.com/font-awesome/4.0.3/css/font-awesome.min.css", rel: "stylesheet" }));
			}
			}
			...
			if(elm.js && !elm.widgets){
				...
				if(!window.jQuery){
					veneer.include(["https://code.jquery.com/jquery-latest.min.js"]);
				}
			}
	...
	events: {
		update: function upd(e){ 
			...
			function render(){
				...
				try{elm.innerHTML=dateDiff(new Date(elm.time-0.1?+elm.time:elm.time), new Date(), elm);}catch(y){
 ...
 veneer("veneer-mustache", {
	events: {
		...
		*/
			...
			if(elm._data){
				//if(targ.tagName=="TEMPLATE") targ=targ.parentNode;
 ...
 veneer("veneer-route", {
	...
	events: {
		insert: function ldr(e){ 
			...
			elm.HISTORY=[]; // track this router's activity
			...
			function ohc(e){
				...
				var st=(elm.HISTORY.slice(-1)[0]||"").date || Date.now();
				
				(elm.HISTORY.slice(-1)[0]||loc).dur= +((Date.now()-st)/1000).toFixed(2);
				
				elm.HISTORY.push(loc); // add nav event to route history
				...
				if(e && e.oldURL && 
					(oldSection=(e.oldURL.split("?")[0].split("#")[1] ||"/")) &&				
  ...
  /* todo/future considerations
	hasRoute(strRoute)
	base path - mostly for ajax/rest?
	

//search start. maybe invert and scan for A tags, then search in linked route, to make sure only linked sections can be found:	
var term="easy";
veneer.$("[route]").filter(function(a){
   "use strict";
   return a.textContent.toLowerCase().indexOf(this)!==-1;
}, term.toLowerCase())

.map(function(a){
  var sum=veneer.$("h1,h2,h3,h4", a).map(veneer.pluck, "textContent").join(" - ").link("#"+ a.getAttribute("route"));;
 sum+="\n :: "+ a.textContent.match(RegExp( "[\\w\\W]{0,30}"+term+"[\\w\\W]{0,20}", "i"))[0].replace(term, term.big());
return sum
...
/**  veneer.js custom html tag definition
 {
	...
	attribs: { 
		...
		refresh: ' # of minutes to wait before updating the content',
...
/**  veneer.js custom html tag definition
 {
	...
	attribs: { 
		...
		inner: ' if present, puts the wrapped element in the deepest element in the wrapper'
	},
	events:  [ 'update' ]
 }
**/
