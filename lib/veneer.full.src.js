//veneer.full.src.js
//110,296b of code compiled Monday, February 16, 2015 1:57:59 AM
...
events: {
remove: function(e){clearInterval( e.target._timer); },
insert: function(e){
	var elm=e.target;
	...
	if(!elm.src){				
		...
		veneer.include(src);
		if(elm.interval){ clearInterval(elm._timer); elm._timer=setInterval(veneer.include.bind(this, src), elm.interval*1000); }
	}
}
},
props:{
		...
		insert: function upd(e){ 
			...
			[["loadstart","loading"],["pause","paused"],["seeking","seeking"],["loadedmetadata","loaded"]].map(function(o){
				elm._player.addEventListener(o[0], function(){
					elm.status=o[1];
					sel.value=elm._player.currentSrc;
				});
			});			
...
/**  veneer.js custom html tag definition
 {
	...
	attribs: { 
		...
		loaded: 'readonly - set when feed is loaded'
	},
	events:  [ 'update' ]
 }
**/
...
events: {
  ...
  console.info("update", e, arguments.callee.caller);
		if(!code) return;
		
		elm.cancel=function(){clearInterval(elm._timer);};
		var FN=Function("event", "console.warn(event);with(this){"+code+"}").bind(elm, e);
